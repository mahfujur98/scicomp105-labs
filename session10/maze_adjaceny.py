...
def init_adjmat(maze):
    adj = np.zeros(10000, dtype=np.bool).reshape((100, 100))
    for y in range(10):
        for x in range(10):
            cell = maze[y, x]
            if y > 0:
...
def plot_cell_walls(ax, maze):
    for y in range(10):
        ...
        for x in range(10):
            ...
            cell = maze[y, x]
            if cell & 1 == 1:
                lc = LineCollection([[(left, top), (right, top)]],
                                    color='black', linewidth=3)
                ax.add_collection(lc)
            if cell & 2 == 2:
                lc = LineCollection([[(right, bottom), (right, top)]],
                                    color='black', linewidth=3)
                ax.add_collection(lc)
            if cell & 4 == 4:
                lc = LineCollection([[(left, bottom), (right, bottom)]],
                                    color='black', linewidth=3)
                ax.add_collection(lc)
            if cell & 8 == 8:
                lc = LineCollection([[(left, bottom), (left, top)]],
                                    color='black', linewidth=3)
                ax.add_collection(lc)
...
def plot_steps(ax, steps):
    for s in steps:
        ...
        ax.add_collection(PatchCollection([patch],
                                          facecolor='blue'))
...
if __name__ == "__main__":
    file_name = None
    if len(sys.argv) == 1:
        file_name = os.path.dirname(sys.argv[0]) + "/maze1.csv"
    else:
